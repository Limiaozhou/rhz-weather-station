...
//USART_Puts(UART4,"Evn_Packagedat",strlen("Evn_Packagedat"));
	unsigned char addr[8] = { 0x42,0x47,0x52,0x03,0x00,0x02,0x00,0x00 };
	...
	psenddat->data.len = 6*2;//十进制6,多少种检测参数，一种6字节
...
//	mem_copy(psenddat->data.yang1.dat, (unsigned char*)&(wat_fer.yang1), 4);
    ...
    psenddat->data.iccard_id.key[0] = 0;
	psenddat->data.iccard_id.key[1] = 218;
	mem_copy(psenddat->data.iccard_id.dat, (unsigned char*)&(sensor.iccard_id), 4);
    
    psenddat->data.iccard_state.key[0] = 0;
	psenddat->data.iccard_state.key[1] = 220;
	mem_copy(psenddat->data.iccard_state.dat, (unsigned char*)&(sensor.iccard_state), 4);
